...
#%% 分别使用第一种以及第二种编码方式验证编码方式的影响,选取最优编码方式进行下一步操作
#train_data=pd.read_csv('S7_Table.csv')
#train_label=np.array(train_data[['observed']]>0).astype(int)
...
#print(np.mean(rocauc1))      #结果为0.957
#for i in range(0,100):
#    roc_aucs=[]
...
#print(np.mean(rocauc2)) #结果为0.953
#for i in range(0,100):
#    roc_aucs=[]
...
clf=GridSearchCV(xgb_model,param,cv=5,scoring='roc_auc',ev)
clf.fit(train_data,train_label)
best_model=clf.best_estimator_
#%%
xgb_mod=xgb.XGBClassifier(learning_rate=0.07,max_depth=5,n_estimators=97,reg_lambda=0.08,min_child_weight=1,n_jobs=4)   # learning _rate 0.07,max_depth=5,reg_lambda=0.08
clf=xgb_mod.fit(train_data,train_label,eval_set=[(train_data,train_label),(validation_data,validation_label)],eval_metric='auc',verbose=False)
clf.evals_result()
s=clf.predict_proba(validation_data)[:,1]
roc_auc_score(validation_label,s)
# precision,recall,_=precision_recall_curve(validation_label,s)
...
# }
# newclf=GridSearchCV(best_model,param,cv=5,scoring='roc_auc')
# best_model=newclf.fit(train_data,train_label)
# #%%
# param={
#
...
#        'reg_lambda':[0.05,0.1,1,2,3]
# }
# newclf2=GridSearchCV(best_model,param,cv=5,scoring='roc_auc')
# best_model=newclf2.fit(train_data,train_label)
...
#     rocauc=[]
#     for train_index,test_index in kfold.split(train_score):
#         xgb_model,predictions,predict_prob,fpr,tpr,precision,recall,roc_auc,prc_auc=xgboost_predict(train_score[train_index][:,(0,1)],train_label[train_index],train_score[test_index][:,(0,1)],train_label[test_index])
#         rocauc.append(roc_auc)
#     roc_aucs.append(np.mean(rocauc))
# cctop_optcd_roc=np.mean(roc_aucs)
# roc_aucs=[]
# for i in range(100):
#     rocauc=[]
#     for train_index,test_index in kfold.split(train_score):
#         xgb_model,predictions,predict_prob,fpr,tpr,precision,recall,roc_auc,prc_auc=xgboost_predict(train_score[train_index][:,(0,2)],train_label[train_index],train_score[test_index][:,(0,2)],train_label[test_index])
#         rocauc.append(roc_auc)
#     roc_aucs.append(np.mean(rocauc))
# cctop_cfd_roc=np.mean(roc_aucs)
# roc_aucs=[]
# for i in range(100):
#     rocauc=[]
#     for train_index,test_index in kfold.split(train_score):
#         xgb_model,predictions,predict_prob,fpr,tpr,precision,recall,roc_auc,prc_auc=xgboost_predict(train_score[train_index][:,(1,2)],train_label[train_index],train_score[test_index][:,(1,2)],train_label[test_index])
#         rocauc.append(roc_auc)
#     roc_aucs.append(np.mean(rocauc))
# cctop_cristaweb_roc=np.mean(roc_aucs)
# roc_aucs=[]
# for i in range(100):
#     rocauc=[]
#     for train_index,test_index in kfold.split(train_score):
...
#         rocauc.append(roc_auc)
#     roc_aucs.append(np.mean(rocauc))
# optcd_cdf_roc=np.mean(roc_aucs)
# roc_aucs=[]
# for i in range(100):
#     rocauc=[]
#     for train_index,test_index in kfold.split(train_score):
#         xgb_model,predictions,predict_prob,fpr,tpr,precision,recall,roc_auc,prc_auc=xgboost_predict(train_score[train_index][:,(1,3)],train_label[train_index],train_score[test_index][:,(1,3)],train_label[test_index])
#
#         rocauc.append(roc_auc)
#     roc_aucs.append(np.mean(rocauc))
# optcd_cristaweb_roc=np.mean(roc_aucs)
# roc_aucs=[]
# for i in range(100):
#     rocauc=[]
#     for train_index,test_index in kfold.split(train_score):
#         xgb_model,predictions,predict_prob,fpr,tpr,precision,recall,roc_auc,prc_auc=xgboost_predict(train_score[train_index][:,(2,3)],train_label[train_index],train_score[test_index][:,(2,3)],train_label[test_index])
#
#         rocauc.append(roc_auc)
#     roc_aucs.append(np.mean(rocauc))
# cfd_cristaweb_roc=np.mean(roc_aucs)
# roc_aucs=[]
# for i in range(100):
#     rocauc=[]
#     for train_index,test_index in kfold.split(train_score):
#         xgb_model,predictions,predict_prob,fpr,tpr,precision,recall,roc_auc,prc_auc=xgboost_predict(train_score[train_index][:,(0,1,2)],train_label[train_index],train_score[test_index][:,(0,1,2)],train_label[test_index])
#
#         rocauc.append(roc_auc)
#     roc_aucs.append(np.mean(rocauc))
# cctop_optcd_cfd_roc=np.mean(roc_aucs)
# roc_aucs=[]
# for i in range(100):
#     rocauc=[]
#     for train_index,test_index in kfold.split(train_score):
#         xgb_model,predictions,predict_prob,fpr,tpr,precision,recall,roc_auc,prc_auc=xgboost_predict(train_score[train_index][:,(0,1,3)],train_label[train_index],train_score[test_index][:,(0,1,3)],train_label[test_index])
#
#         rocauc.append(roc_auc)
#     roc_aucs.append(np.mean(rocauc))
# cctop_optcd_cristaweb_roc=np.mean(roc_aucs)
# roc_aucs=[]
# for i in range(100):
#     rocauc=[]
#     for train_index,test_index in kfold.split(train_score):
#         xgb_model,predictions,predict_prob,fpr,tpr,precision,recall,roc_auc,prc_auc=xgboost_predict(train_score[train_index][:,(0,2,3)],train_label[train_index],train_score[test_index][:,(0,2,3)],train_label[test_index])
#
#         rocauc.append(roc_auc)
#     roc_aucs.append(np.mean(rocauc))
# cctop_cfd_cristaweb_roc=np.mean(roc_aucs)
# for i in range(100):
#     rocauc=[]
#     for train_index,test_index in kfold.split(train_score):
#         xgb_model,predictions,predict_prob,fpr,tpr,precision,recall,roc_auc,prc_auc=xgboost_predict(train_score[train_index][:,(1,2,3)],train_label[train_index],train_score[test_index][:,(1,2,3)],train_label[test_index])
#
#         rocauc.append(roc_auc)
#     roc_aucs.append(np.mean(rocauc))
# optcd_cfd_cristaweb_roc=np.mean(roc_aucs)
#
# rocauc=[]
# prcauc=[]
# for i in range(0,100):
...
#     prc_aucs=[]
#     for train_index,test_index in kfold.split(train_score):
#         xgb_model,predictions,predict_prob,fpr,tpr,precision,recall,roc_auc,prc_auc=xgboost_predict(train_score[train_index],train_label[train_index],train_score[test_index],train_label[test_index])
#         roc_aucs.append(roc_auc)
#         prc_aucs.append(prc_auc)
#     rocauc.append(np.mean(roc_aucs))
#     prcauc.append(np.mean(prc_aucs))
# print("the auc is %d" % (np.mean(rocauc)))
# print("the prc is %d" % (np.mean(prcauc)))


    
#%%使用序列加分数进行训练，可以看到第三种编码方式同样得到了更好的效果
import matlab.engine
